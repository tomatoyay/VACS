...
func load_theme(theme: ConfigFile):
	# Text
	var theme_font = DialogicUtil.path_fixer_load(theme.get_value('text', 'font', 'res://addons/dialogic/Example Assets/Fonts/DefaultMenuFont.tres'))
	...
	# Backgrounds
	TextureRectElement.texture = null
	ColorRectElement.color = Color(theme.get_value('background','color', "00ffffff"))

	if theme.get_value('background', 'modulation', false):
		TextureRectElement.modulate = Color('00ffffff')
	else:
		TextureRectElement.modulate = Color('00ffffff')
