...
static func auto_skip_on(toggle: bool, delay : float=0.0000000000000000000000001):
	if has_current_dialog_node():
		var dialog_node = Engine.get_main_loop().get_meta('latest_dialogic_node')
		dialog_node.autoSkipMode = toggle
		dialog_node.autoWaitTime = float(delay)
		dialog_node.get_node('TextBubble').skip = 0
	else:
		print('[D] Tried to toggle auto advance mode, but no dialog node exists.')
...
static func _set_InGameMenu_Button(toggle: bool):
	if has_current_dialog_node():
		var dialog_node = Engine.get_main_loop().get_meta('latest_dialogic_node')
		if toggle == true:
			dialog_node.in_game_button = true
		elif toggle == false:
			dialog_node.in_game_button = false
