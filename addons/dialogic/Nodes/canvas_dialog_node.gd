...
signal auto_skip_toggled(toggle_value)
...
func set_dialog_node_scene(scene) -> void:
	...
	if dialog_node:
		...
		assert(_err == OK)
		_err = dialog_node.connect("auto_skip_toggled", self, "_on_auto_skip_toggled")
		assert(_err == OK)
...
func _on_auto_skip_toggled(toggle_value) -> void:
	emit_signal("auto_skip_toggled", toggle_value)
