...
var autoSkipMode: bool = false setget _set_autoSkipMode
...
func _process(delta):
	...
	if current_event.has('text'):
		if '[nw]' in current_event['text'] or '[nw=' in current_event['text'] or noSkipMode or autoPlayMode or autoSkipMode:
...
func _input(event: InputEvent) -> void:
	if not Engine.is_editor_hint() and event.is_action_pressed(Dialogic.get_action_button()) and autoPlayMode:
		...
		get_node("/root/MainScene/DialogicVisualNovelManager/Menus/MenuContainer")._on_Auto_Button_toggled(false)
		autoPlayMode = false
		return
	
	if not Engine.is_editor_hint() and event.is_action_pressed(Dialogic.get_action_button()) and autoSkipMode:
		emit_signal("auto_skip_toggled", false)
	
		get_node("/root/MainScene/DialogicVisualNovelManager/Menus/MenuContainer")._on_Skip_Button_toggled(false)
		$TextBubble.skip = 0
		autoSkipMode = false
		return
...
func _on_text_completed():
	...
	if current_event.has('options'):
		...
		if current_event['event_id'] == 'quick_wit':
			var quick_wit_handler = $CustomEvents.handlers[current_event['event_id']]
			if autoSkipMode == true:
				$TextBubble.skip = 0
				emit_signal("auto_skip_toggled", false)
				get_node("/root/MainScene/DialogicVisualNovelManager/Menus/MenuContainer")._on_Skip_Button_toggled(false)
			...
			if autoPlayMode == true:
				...
				get_node("/root/MainScene/DialogicVisualNovelManager/Menus/MenuContainer")._on_Auto_Button_toggled(false)
				autoPlayMode = false
			quick_wit_handler.handle_event(current_event, self)
		...
		var in_game_menu =  get_node("/root/MainScene/DialogicVisualNovelManager/Menus/MenuContainer/InGameMenu")
		if in_game_menu.visible == true:
			var auto_button = get_node("/root/MainScene/DialogicVisualNovelManager/Menus/MenuContainer/InGameMenu/Auto_Button")
			var skip_button = get_node("/root/MainScene/DialogicVisualNovelManager/Menus/MenuContainer/InGameMenu/Skip_Button")
		...
		'dialogic_002':
			...
			if event['character'] == '':# No character found on the event. Skip.
				...
				if event.get('type', 0) == 0 and not portrait_exists(character_data):
					...
					if autoSkipMode == true:
						p.animate(event.get('animation', '[No Animation]'), 0)
					else:
					...
					if event['character'] == '[All]':
						...
						for p in $Portraits.get_children():
							if is_instance_valid(p) and p.character_data['file'] == event['character']:
								event = insert_animation_data(event, 'leave', 'fade_out_down.gd')
								if autoSkipMode == true:
									p.animate(event.get('animation', 'instant_out.gd'), 0, 1, true)
								else:
					...
					if portrait_exists(character_data):
						for portrait in $Portraits.get_children():
							if portrait.character_data.get('file', true) == character_data.get('file', false):
								...
								if autoSkipMode:
									portrait.animate(event.get('animation', '[No Animation]'), 0.5, 0)
								else:
		...
		_:
			if event['event_id'] in $CustomEvents.handlers.keys():
				...
				if event['event_id'] == 'quick_wit':
					...
					update_text(event['question'])
				# Choice event
				elif event['event_id'] == 'quick_wit_choice':
					var quick_wit_handler = $CustomEvents.handlers['quick_wit']
					quick_wit_handler.stop_event(event, self)
					emit_signal("event_start", "choice", event)
...
func answer_question(i, event_idx, question_idx):
	if current_event['event_id'] == 'quick_wit':
		var quick_wit_handler = $CustomEvents.handlers['quick_wit']
		quick_wit_handler.stop_event(current_event, self)
...
#special function for quick wit
func answer_quick_wit_question(event_idx, question_idx, text):
	play_audio("selecting")
	...
	if record_history:
		HistoryTimeline.add_answer_to_question(str(text))
...
func get_custom_choice_button(label: String):
	var quick_wit = "res://art/Quick Wit Button.png"
	var quick_hover = "res://art/Quick Wit Hover.png"

	var custom_button = [true, [1, 1, 1, 1], false, [0, 0, 0, 1], true, quick_wit, true, [1, 1, 1, 1]]
	var custom_hover = [true, [1, 1, 1, 1], false, [0, 0, 0, 1], true, quick_hover, true, [1, 1, 1, 1]]
	...
	# Different styles
	var default_background = "res://art/Quick Wit Hover.png"
	var default_style = [
	...
	var hover_style = [true, Color( 0.698039, 0.698039, 0.698039, 1 ), false, Color.black, true, default_background, false, Color.white]
		...
		# Create StyleBoxTextures for different button states
	var style_normal = StyleBoxTexture.new()
	style_normal.texture = load(quick_wit)

	var style_hover = StyleBoxTexture.new()
	style_hover.texture = load(quick_hover)
	
	var style_focus = custom_hover
...
func get_classic_choice_button(label: String):
	...
	# Different styles
	var default_background = 'res://art/Choice Button Texture.png'
	var default_style = [
		false,               # 0 $TextColor/CheckBox
		Color.white,         # 1 $TextColor/ColorPickerButton
		false,               # 2 $FlatBackground/CheckBox
...
func _set_autoSkipMode(newValue : bool):
	emit_signal("auto_skip_toggled", newValue)
	autoSkipMode = newValue
	if newValue == true:
		$TextBubble.skip()
	if newValue == false:
		$TextBubble.skip = 0
