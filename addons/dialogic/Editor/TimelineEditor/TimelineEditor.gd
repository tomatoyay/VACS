...
# the Quick Wit button adds multiple blocks 
func _on_ButtonQuickWit_pressed() -> void:
	var at_index = -1
	if selected_items:
		at_index = selected_items[-1].get_index()+1
	else:
		at_index = timeline.get_child_count()
	TimelineUndoRedo.create_action("[D] Add quick wit events.")
	TimelineUndoRedo.add_do_method(self, "create_quickwit", at_index)
	TimelineUndoRedo.add_undo_method(self, "remove_events_at_index", at_index, 4)
	TimelineUndoRedo.commit_action()

func create_quickwit(at_position):
	if at_position == 0: selected_items = []
	else: selected_items = [timeline.get_child(at_position-1)]
	if len(selected_items) != 0:
		# Events are added bellow the selected node
		# So we must reverse the adding order
		create_event("dialogic_013", {'no-data': true}, true)
		create_event("quick_wit_choice", {'no-data': true}, true)
		create_event("quick_wit_choice", {'no-data': true}, true)
		create_event("quick_wit", {'no-data': true}, true)
	else:
		create_event("quick_wit", {'no-data': true}, true)
		create_event("quick_wit_choice", {'no-data': true}, true)
		create_event("quick_wit_choice", {'no-data': true}, true)
...
func update_custom_events() -> void:
	...
	for custom_event_id in custom_events.keys():
		...
		if button.event_id == 'quick_wit':
			button.connect('pressed', self, "_on_ButtonQuickWit_pressed", [])
		else:
			button.connect("pressed", self, "_create_event_button_pressed", [custom_event_id])
...
func indent_events() -> void:
	...
	for event in event_list:
		...
		elif event.event_data['event_id'] == 'quick_wit':
			indent += 1
			starter = true
			question_index += 1
			question_indent[question_index] = indent
		elif event.event_data['event_id'] == 'quick_wit_choice':
			if question_index > 0:
				indent = question_indent[question_index] + 1
				starter = true
