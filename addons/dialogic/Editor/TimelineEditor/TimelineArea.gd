...
func _draw():
	...
	for event in $TimeLine.get_children():
		...
		if event.event_name == 'Question' or event.event_name == 'Condition' or event.event_name == 'Quick Wit':
			var keep_going = true
			var end_reference
					...
					if e.get_index() > event.get_index():
						if e.current_indent_level == event.current_indent_level:
							if e.event_name == 'End Branch':
								end_reference = e
								keep_going = false
							if e.event_name == 'Question' or event.event_name == 'Condition' or e.event_name == "Quick Wit":
								keep_going = false
			if keep_going == false:
				if end_reference:
		...
		if event.current_indent_level > 0:
			...
			if next_event.event_name != 'End Branch' and (event.event_name != 'Choice' or event.event_name != 'Quick Wit Choice'):
				if event.event_name != 'Question' and next_event.event_name == 'Choice':
					# Skip drawing lines before going to the next choice
					pass
				elif event.event_name != 'Quick Wit' and next_event.event_name == 'Quick Wit Choice':
					# Skip drawing lines before going to the next choice
					pass
				else:
					draw_rect(Rect2(
		...
		# Drawing arc
		if event.event_name == 'Choice' or event.event_name == 'Quick Wit Choice':
			...
			if next_event.event_name == "Choice" or next_event.event_name == "Quick Wit Choice" or next_event.event_name == "End Branch":
				continue
