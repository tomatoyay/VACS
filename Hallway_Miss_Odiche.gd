...
func _on_Carpet_Area2D_input_event(_viewport,event,_shape_idx):
	if mouse_over and event.is_pressed() and event.button_index == BUTTON_LEFT:
		var dialog = Dialogic.start("Carpet")
		add_child(dialog)
...
func _on_Radiator_Area2D_input_event(_viewport,event,_shape_idx):
	if mouse_over and event.is_pressed() and event.button_index == BUTTON_LEFT:
		var dialog = Dialogic.start("Radiator")
		add_child(dialog)
		yield(dialog,'timeline_end')
		var key = Dialogic.get_variable('key')
		if key == '1':
...
func _on_Lamp_Area2D_input_event(_viewport,event,_shape_idx):
	if mouse_over and event.is_pressed() and event.button_index == BUTTON_LEFT:
		var dialog = Dialogic.start("Lamp")
		add_child(dialog)
		yield(dialog,'timeline_end')
...
func _on_Window_pressed():
	var dialog = Dialogic.start("Window")
	add_child(dialog)
	yield(dialog,'timeline_end')
...
func _on_Painting_pressed():
	var dialog = Dialogic.start("Painting")
	add_child(dialog)
	yield(dialog,'timeline_end')
	var hook = Dialogic.get_variable('painting')
	if hook == '1':
...
func _on_Door_Crack_pressed():
	var dialog = Dialogic.start("Door Crack")
	add_child(dialog)
	yield(dialog,'timeline_end')
...
func _on_Doorknob_pressed():
	var dialog = Dialogic.start("Doorknob")
	dialog.connect("dialogic_signal", self, "_on_dialogic_signal")
	add_child(dialog)
	yield(dialog,'timeline_end')
